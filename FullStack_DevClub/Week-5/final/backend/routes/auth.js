...
router.post('/register', async (req, res) => {
    ...
    if (error) return res.status(400).send({ errors: { message: error.details[0].message } });

    // if (!data.username) {
    //     return res.status(400).json({ errors: { username: { message: "Username Required" } } });
    // }
    // if (!data.email) {
    //     return res.status(400).json({ errors: { email: { message: "Email Required" } } });
    // }
    // if (!data.password) {
    //     return res.status(400).json({ errors: { password: { message: "Password Required" } } });
    // }
    // if (data.password.length < 6) {
    //     return res.status(400).json({ errors: { password: { message: "Password Length less than 6" } } });
    // }
