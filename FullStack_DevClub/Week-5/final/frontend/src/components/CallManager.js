...
class CustomAlert {
    constructor(role) {
        this.render = function (dialog) {
            ...
            var dialogoverlay = document.getElementById(`dialogoverlay${role}`);
            var dialogbox = document.getElementById(`dialogbox${role}`);
            dialogoverlay.style.display = "block";
            dialogoverlay.style.height = winH + "px";
            console.log(dialogbox.clientWidth)
            dialogbox.style.top = "100px";
            dialogbox.style.display = "block";
            dialogbox.style.left = (winW / 2) - (dialogbox.clientWidth * .5) + "px";
        ...
        this.ok = function () {
            ...
            document.getElementById(`dialogoverlay${role}`).style.display = "none";
        };
    }
}
...
const CallSender = ({ partnerId }) => {
    ...
    const sendMessage = async (callMsg) => {
        if (!(currentUser && partnerId)) return
        const msgId = `${currentUser.uid}-${partnerId}-${Date.now()}`
            ...
            onValue(callRef, async (snapshot) => {
                ...
                if (data) {
                    if (data === "declined") {
                        ...
                        setKey(null)
                        sendMessage(`${callData.callType} Call Declined`)
                    ...
                    if (data === "ongoing") {
                        ...
                        sendMessage(`${callData.callType} Call Finished`)
    ...
    const endCall = async () => {
        ...
        sendMessage(`Missed ${callData.callType} Call`)
    }
