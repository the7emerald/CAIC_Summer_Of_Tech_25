...
import { getDatabase, set, ref, remove, get } from "firebase/database";
...
export const handleDelete = async (email, password) => {
  try {
    const user = auth.currentUser;
    const userDocRef = ref(database, `ChatUsers/${user.uid}`);
    const userChatRef = ref(database, `ChatList/${user.uid}`);
...
export const getName = () => {
  return new Promise(async (resolve, reject) => {
    const userRef = ref(database, `ChatUsers/${auth.currentUser.uid}/username`)
    await get(userRef)
      .then((snap) => {
        if (snap.exists()) {
